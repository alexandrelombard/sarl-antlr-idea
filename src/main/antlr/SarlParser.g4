parser grammar SarlParser;

// I used to import KotlinParser but it causes issues, so the grammars are manually merged

options { tokenVocab = SarlLexer; }
...
skillDeclaration
    : modifierList? (SKILL) NL* simpleIdentifier
    (NL* typeParameters)? (NL* primaryConstructor)?
    (NL* COLON NL* delegationSpecifiers)?
    (NL* typeConstraints)?
    (NL* classBody | NL* enumClassBody)?
...
agentBody
    : LCURL NL* agentMemberDeclaration* NL* RCURL
    ;

behaviorBody
    : LCURL NL* behaviorMemberDeclaration* NL* RCURL
    ;
